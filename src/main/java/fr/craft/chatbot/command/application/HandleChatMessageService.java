...
public class HandleChatMessageService {
  ...
  public void handle(ChatMessage message) {
    CommandName.parse(message.content())
      .map(this::resolveOutcome)
      .map(translator::translate)
      .ifPresent(response -> chatMessagePublisher.send(response.value()));
  }

  private CommandOutcome resolveOutcome(CommandName askedCommand) {
    return commandRepository.findAll().outcomeFor(askedCommand);
  }
}
